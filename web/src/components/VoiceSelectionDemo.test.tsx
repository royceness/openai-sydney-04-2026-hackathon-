...
function renderVoiceSelectionDemo({
  ...
  onDraftCommentAtLocation = vi.fn(() => Promise.resolve({ status: "created" as const })),
  ...
  onDraftCommentAtLocation?: (body: string, context: CodeSelection) => Promise<{ status: "created" | "empty" | "failed"; message?: string }>;
  onEditComment?: (
    commentId: string,
    body: string,
...
describe("VoiceSelectionDemo", () => {
  ...
  it("reads an auto-generated initial analysis thread by category", () => {
    const summaryThread: ReviewThread = {
      ...completedThread,
      id: "thr_summary",
      source: "init",
      title: "PR summary",
      markdown: "This PR updates the voice agent.\nStart with VoiceSelectionDemo.",
    };

    expect(readInitialAnalysisThreadForVoice([summaryThread], "pr-summary")).toEqual({
      ok: true,
      category: "pr-summary",
      title: "PR summary",
      found: true,
      ...
      markdown: "This PR updates the voice agent.\nStart with VoiceSelectionDemo.",
      totalLines: 2,
      message: "PR summary is complete.",
    });
    expect(readInitialAnalysisThreadForVoice([], "bug-finder")).toEqual({
      ok: true,
      category: "bug-finder",
      title: "Bug finder",
      found: false,
      ...
      markdown: null,
      totalLines: 0,
      message: "Bug finder is not loaded yet.",
    });
  });
    ...
    expect(createVoiceControlController).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        tools: expect.arrayContaining([
          ...
          expect.objectContaining({
            name: "draft_pr_comment_at_location",
          }),
          expect.objectContaining({
          ...
          expect.objectContaining({
            name: "read_initial_analysis_thread",
          }),
          expect.objectContaining({
  ...
  it("instructs the realtime model to stay quiet unless directly answering", () => {
    ...
    expect(options.instructions).toContain('Before answering a high-level question like "give me a summary of what\'s changed in this PR", "give me an overview", "where should I dive in", "what changed", or "what should I look at first", you MUST call read_initial_analysis_thread with category pr-summary');
    expect(options.instructions).toContain("Before answering a general tests or test coverage question, you MUST call read_initial_analysis_thread with category tests-audit");
    expect(options.instructions).toContain("Before answering a general architecture, design, structure, or integration question, you MUST call read_initial_analysis_thread with category architecture-coherence-report");
    expect(options.instructions).toContain("Before answering a general bug, risk, regression, edge case, or failure-mode question, you MUST call read_initial_analysis_thread with category bug-finder");
    expect(options.instructions).toContain("Before answering a general docs, README, comment, or documentation question, you MUST call read_initial_analysis_thread with category doc-validator");
    expect(options.instructions).toContain("If that thread is found, you MUST consider its Markdown before answering");
    expect(options.instructions).toContain("When the user asks to add PR comments from testing gaps or findings in a Codex thread, first read the named thread");
    expect(options.instructions).toContain("Use the file and line numbers from that thread when they point to files changed in this PR, and call draft_pr_comment_at_location for each comment");
    expect(options.instructions).toContain("If a thread line number refers to a file not changed in this PR, do not attach a comment to that unchanged file");
    expect(options.instructions).toContain("use list_pr_files, summarize_changed_lines, and read_pr_file_range to find the relevant changed PR logic that is not tested");
  ...
  it("drafts a PR comment at an explicit changed location when the realtime tool executes", async () => {
    const onDraftCommentAtLocation = vi.fn(() => Promise.resolve({ status: "created" as const }));
    const options = renderVoiceSelectionDemo({ onDraftCommentAtLocation });
    const draftAtLocation = options.tools.find((tool) => tool.name === "draft_pr_comment_at_location");
    if (!draftAtLocation) {
      throw new Error("Expected location-aware PR comment voice tool");
    }

    await expect(
      draftAtLocation.execute({
        filePath: "diagram.ts",
        startLine: 201,
        endLine: 202,
        comment: "This path needs a regression test.",
      }),
    ).resolves.toEqual({
      ok: true,
      status: "created",
    ...
    expect(onDraftCommentAtLocation).toHaveBeenCalledWith("This path needs a regression test.", {
      filePath: "src/review/diagram.ts",
      side: "new",
      startLine: 201,
      endLine: 202,
    ...
    expect(await screen.findByText("PR comment drafted")).toBeInTheDocument();
    expect(screen.getByText("Lines 201-202 of src/review/diagram.ts")).toBeInTheDocument();
  });
  ...
  it("lists, reads, and searches currently loaded thread text when realtime tools execute", async () => {
    const summaryThread: ReviewThread = {
      ...
      source: "init",
      title: "PR summary",
      markdown: "Summary says the voice agent changed.",
    };
    const options = renderVoiceSelectionDemo({ threads: [summaryThread, completedThread] });
    ...
    const readInitialAnalysis = options.tools.find((tool) => tool.name === "read_initial_analysis_thread");
    const searchThreads = options.tools.find((tool) => tool.name === "search_review_threads");
    if (!listThreads || !getThreadText || !readInitialAnalysis || !searchThreads) {
    ...
    expect(readInitialAnalysis.execute({ category: "pr-summary" })).toEqual({
      ok: true,
      category: "pr-summary",
      title: "PR summary",
      found: true,
    ...
    expect(screen.getByText(/Summary says the voice agent changed\./)).toBeInTheDocument();
